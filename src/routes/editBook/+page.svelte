<script>
    ...
    $: {
        ...
        originalBookId = params.get('bookId') || "";
        // Fetch book details using `originalBookId` here (placeholder logic)
        if (originalBookId && bookTitle === "") {
            // Simulate fetching book data
            bookTitle = "Thanh Gươm Diệt Quỷ";
            author = "Gotouge Koyoharu";
            bookId = originalBookId;
            selectedCategory = "Shounen";
            tags = ["Action", "Fantasy"];
            coverPreview = "/path/to/cover.jpg";
        }
    }
    ...
    async function saveBook() {
        ...
        formData.append("originalBookId", originalBookId);
        if (coverImage) {
...
<div class="bg-blue-50 min-h-screen p-6">
    <header class="flex justify-between items-center mb-4">
        <h1 class="text-2xl font-bold text-blue-600">Chỉnh Sửa Sách</h1>
