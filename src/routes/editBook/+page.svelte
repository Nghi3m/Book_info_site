<script>
            ...
            if (bookId) {
                ...
                if (resBook.ok) {
                    ...
                    if (bookData.success && bookData.book) {
                        ...
                        if (book.coverPath) {
                            coverPreview = `http://localhost:3000/${book.coverPath}`;
                        }
                        if (book.filePath) {
    ...
    async function saveBook() {
        ...
        if (coverImage) {
            formData.append("cover", coverImage); // New uploaded file
        } else if (coverPreview) {
            formData.append("coverFilename", coverPreview.split('/').pop()); // Existing filename
        }

        if (pdfFile) {
            formData.append("pdf", pdfFile); // New uploaded file
        } else if (pdfFileName) {
            formData.append("pdfFilename", pdfFileName); // Existing filename
        }
...
<div class="bg-blue-50 min-h-screen p-6">
    <header class="flex justify-between items-center mb-4">
        <h1 class="text-2xl font-bold text-blue-600">Chỉnh sửa</h1>
        <button class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600" on:click={() => goto('/manageBookOption')}>Quay Lại</button>
    </header>
                    ...
                    <div class="flex flex-wrap gap-2 mt-2">
                        {#each categories as category}
                            <label class="flex items-center">
                                <input type="checkbox" value={category.category_id} bind:group={selectedCategories} />
